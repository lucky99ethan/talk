...
const Release = () => {
  const [releaseList, setReleaseList] = useState<Movie[]>([]);
  const [currentPage, setCurrentPage] = useState(0);
  const itemsPerPage = 4;

  const getRelease = () => {
    fetch(
      "https://api.themoviedb.org/3/movie/now_playing?api_key=1911927029d548114049751dcaf5dbb0"
    )
      .then((res) => res.json())
      .then((json) => {setReleaseList(json.results);
  ...
  const handleNextPage = () => {
    if ((currentPage + 1) * itemsPerPage < releaseList.length) {
  ...
  const currentItems = releaseList.slice(startIndex, endIndex);

  return (
    <div className="bg-black text-white py-4 px-6 relative">
      <h1 className="text-2xl font-bold mb-4">Just Released</h1>
      ...
      {(currentPage + 1) * itemsPerPage < releaseList.length && (
        <button
